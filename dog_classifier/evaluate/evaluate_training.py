...
def prob_multiclass(Y_pred, Y_test, label, path):
...
def predict(path_to_file, path_to_save, encoder_model, fname):
    ...
    testDataloader = DataGenerator(df_test,
                                   encoder_model=encoder_model, shuffle=True)

    # Test predicten
    Y_pred = model.predict_generator(testDataloader, verbose=1)

    np.savetxt(path_to_save + fname, Y_pred)


def preprocess(path_to_file, encoder_model):
    ...
    Y_pred = np.genfromtxt(path_to_file)
    ...
    return Y_pred, Y_test, Y_cls, Y_true
